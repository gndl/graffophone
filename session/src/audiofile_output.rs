...
impl Context {
    pub fn new(codec_name: &str, in_sample_rate: usize, out_sample_rate: usize, channels: usize, file_path: &str,) -> Result<Context, failure::Error> {
    ...
    fn write_header(&mut self) -> Result<(), failure::Error> {
        self.output.write_header().map_err(|e| failure::err_msg(format!("{}", e)))
    }

    fn write_samples(&mut self, channels: &Vec<Vector>, nb_samples_per_channel: usize,) -> Result<(), failure::Error> {
...
impl Output for AudioFileOutput {
    ...
    fn open(&mut self) -> Result<(), failure::Error> {
        ...
        let mut context = Context::new(self.codec_name.as_str(), self.in_sample_rate, self.out_sample_rate, channels, self.file_path.as_str())?;

        if context.channels() != channels {
            self.channel_layout = channel::Layout::from_channels(context.channels()).to_string();
        }
    ...
    fn write(
        ...
        let ctx = self.context.as_mut().ok_or(failure::err_msg(format!("AudioFileOutput not open")))?;

        ctx.write_samples(channels, nb_samples_per_channel)
    }
