...
pub const MODEL: &str = "BoundedTriangle";
//ascent and end of descent

pub struct BoundedTriangle {
    sample_rate: f64,
    current_output: f32,
    dy: f32,
...
impl Talker for BoundedTriangle {
        ...
        while i < ln {
            if i == period_part_end_idx {
                ...
                if dy < 0. {
                    ...
                    let e_y = roof_buf[(ln - 1).min(i + period_part as usize)];
                    dy = (e_y - current_output) / period_part as f32;
                }
                else {
                    period_part = period * (1. - ratio);
                    let e_y = floor_buf[(ln - 1).min(i + period_part as usize)];
                    dy = (e_y - current_output) / period_part as f32;
                }
