...
impl URIDMapper for SimpleMapper {
    ...
    fn unmap(&self, urid: URID) -> Option<&Uri> {
        ...
        uris.get(urid.get() as usize - 1).map(::std::borrow::Borrow::borrow)
    }
}
...
mod test {
    ...
    fn urid_mapper() {
        ...
        assert_eq!(feature_map.map(&"Hello, world!".into_uri()).unwrap().get(), 1);
        assert_eq!(feature_unmap.unmap(NonZeroU32::new(1).unwrap()).unwrap(), &uri);
    }
}
