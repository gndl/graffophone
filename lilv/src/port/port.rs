...
impl<T: PortType> Clone for PortHandle<T> {
    #[inline]
    fn clone(&self) -> Self {
        PortHandle { index: self.index, _marker: PhantomData }
    }
}
...
pub trait Port<'p> {
    ...
    #[inline]
    fn has_class<'u, U: ?Sized>(&'u self, class_uri: &U) -> bool where Uri<'u>: PartialEq<U> {
        self.classes().into_iter().filter(|u| *u == class_uri).next().is_some()
    }
    ...
    fn optional(&self) -> bool {
        self.has_class(unsafe {CStr::from_bytes_with_nul_unchecked(::lilv_sys::LV2_CORE__connectionOptional as &[_])})
    }

    #[inline]
    fn try_handle<PT: PortType>(&self) -> Option<PortHandle<PT>> {
        if self.is::<PT>() {
            Some(PortHandle { index: self.index(), _marker: PhantomData })
        } else {
            None
        }
    }
    ...
    fn direction(&self) -> PortDirection {
        if self.has_class(unsafe { CStr::from_bytes_with_nul_unchecked(::lilv_sys::LV2_CORE__InputPort as &[_]) }) {
            PortDirection::Input
        } else if self.has_class(unsafe { CStr::from_bytes_with_nul_unchecked(::lilv_sys::LV2_CORE__OutputPort as &[_]) }) {
